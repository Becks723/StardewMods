...
namespace FontSettings.Framework.Integrations
{
    ...
    {
        private GenericModConfigMenuFluentHelper _helperCache;
        ...
        {
            LocalizedContentManager.OnLanguageChange += this.OnGameLanguageChanged;

            this._helperCache = helper;
            this.Register(this._helperCache, LocalizedContentManager.CurrentLanguageCode);
        }

        private void OnGameLanguageChanged(LocalizedContentManager.LanguageCode code)
        {
            this._helperCache.Unregister();
            this.Register(this._helperCache, code);
        }

        private void Register(GenericModConfigMenuFluentHelper helper, LocalizedContentManager.LanguageCode language)
        {
            helper.Register()
            ...
            if (FontHelpers.IsCurrentLatinLanguage())
            {
                // enable latin dialogue font
                //   (only show if unde latin language)
                helper.AddCheckbox(
